...
if (!$q) {
    $q = 1;
    go(site."/process.php?process=licenselist");
}

$query = $db->prepare("SELECT * FROM lisanslar WHERE lisans_domain LIKE :adi ORDER BY  lisans_id DESC");
$query->execute([':adi' => '%' .$q. '%']);
...
<div id="wrapper">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
         ...
         $query = $db->prepare("SELECT * FROM lisanslar WHERE lisans_domain LIKE :adi ORDER BY lisans_id DESC LIMIT :show , :lim");
         ...
         ?>
             ...
             <div class="pagination">
                 <ul>
                     <?php
                     if ($total > $lim) {
                         pagination($s, ceil($total / $lim), 'licensesearch.php?q='.$q.'&s=');
                     }
                     ?>
                 </ul>
             </div>
